...
CREATE TABLE `orders` (
  ...
  `id_orederstatus` int(11) NOT NULL
) ENGINE=InnoDB DEFAULT CHARSET=utf8;
...
INSERT INTO `users` (`id_user`, `user_name`, `user_login`, `user_pass`, `user_lastact`) VALUES
...
ALTER TABLE `orders`
  ...
  ADD KEY `orders_ibfk_1` (`id_orederstatus`),
  ADD KEY `id_user` (`id_user`);
...
--
ALTER TABLE `order_status`
  MODIFY `id_orderstatus` int(11) NOT NULL AUTO_INCREMENT, AUTO_INCREMENT=3;
...
ALTER TABLE `basket`
  ...
  ADD CONSTRAINT `basket_ibfk_3` FOREIGN KEY (`id_order`) REFERENCES `orders` (`id_order`);
...
--
ALTER TABLE `orders`
  ADD CONSTRAINT `orders_ibfk_1` FOREIGN KEY (`id_orederstatus`) REFERENCES `order_status` (`id_orderstatus`),
