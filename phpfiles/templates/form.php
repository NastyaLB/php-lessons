...
?>
    ...
    <form action="#" method="POST" enctype="multipart/form-data"><?
        if(!$_SESSION[login]) echo '<p>Введите&nbsp;логин</p><input type="text" name="fio" required><br/>';
        else echo '<input type="hidden" name="fio" value="'.$_SESSION[login].'">';?>	
...
<?  
    ...
    $Nlogin=strip_tags($_POST[fio]);    //  strip_tags() удаляет теги из строки
    $Ncomment=strip_tags($_POST[comment]); 
               
    if(!mysqli_fetch_assoc($res) && $xAns==1) {
        mysqli_query($connect,"INSERT INTO comments (login, text) VALUES ('$Nlogin','$Ncomment')");
        echo 'Комментарий опубликован!<br/>';
    } elseif(mysqli_fetch_assoc($res) && $xAns==0) echo 'Задачка не решена.<br/>Комментарий не опубликован!<br/>';
